...
| TV / sprite head | **3D Video** → subject GLB | Animated GLB (one skinned mesh, e.g. Meshy TV-head) with a **plane parented to a bone** (`headfront`) so you can put stills, sprites or muted video on the screen. Do not paint `Mesh_0` — that replaces the whole body. |
