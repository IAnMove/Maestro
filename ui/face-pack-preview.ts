...
import { EXPRESSIONS, VISEMES } from './src/features/scene3d/speech/types.ts'
import { GLTFLoader } from 'three/addons/loaders/GLTFLoader.js'

const SHOTS = ['hangar-talk', 'sea-talk'] as const
...
for (const id of SHOTS) {
  ...
  button.type = 'button'
  button.textContent = id === 'hangar-talk' ? 'Hangar talk' : 'Sea talk'
...
function select(id: typeof SHOTS[number]) {
  ...
  mount(current)
  caption.textContent = `${id} · ${current.dressing} · ${current.duration}s · synthetic vowels`
...
const tick = (now: number) => {
  const seconds = Number.isFinite(freeze) ? Math.max(0, Math.min(current.duration - 0.01, freeze)) : ((now - started) / 1000) % current.duration
