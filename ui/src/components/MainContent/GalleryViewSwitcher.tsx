...
/** Sits in the gallery's top-right corner, under the navigation. Which layout
 *  suits a set of outputs changes by the minute — dense to find something,
 *  one-up to judge it — so the choice stays visible rather than buried in
 *  settings. The selected view is sunken and the others are raised, the same
 *  grammar the navigation uses. */
export function GalleryViewSwitcher() {
  ...
  return (
    <div
      ...
      className="pointer-events-auto flex items-center gap-0.5 rounded-lg border border-border/70 bg-bg-secondary/90 p-0.5 shadow-lg backdrop-blur"
    >
