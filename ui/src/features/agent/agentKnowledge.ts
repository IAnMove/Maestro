...
- Use review_series_attempts with confirm=true after the user explicitly chooses takes. selected_latest addresses human-visible shot_numbers (shot order, never a take index). all_latest is only valid for choosing pending takes and mirrors “Use pending takes”. Rejecting is deliberately limited to one shot at a time. Supply attempt_id only when the user named a particular historical take on exactly one shot; “use take 2 for this shot” is that attempt’s id, not shot_numbers: [2] alone. Without attempt_id the executor resolves the latest eligible completed attempt of those shot numbers.
