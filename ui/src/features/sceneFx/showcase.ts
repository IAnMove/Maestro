...
export function isFxShowcaseDocument(document: unknown): boolean {
  ...
  if (!Array.isArray(cues) || cues.length < 12) return false
  return cues.every(cue => Boolean(cue && typeof cue === 'object' && String((cue as { id?: unknown }).id ?? '').startsWith('showcase-')))
}

export function showcaseCollectionFrom(document: unknown): 'all' | 'anime' {
  ...
  return kinds.length > 0 && kinds.length <= 12 && kinds.every(kind => anime.has(kind)) ? 'anime' : 'all'
}
