...
test('Character Creator handoff reuses a kit with the same base and otherwise drafts a new one', () => {
  ...
  const collided = kitFromFaceRigHandoff({ name: 'Luma', source: '/api/v1/file/luma-other.png', workspace: 'default' }, library)
  assert.notEqual(collided.id, existing.id)
  assert.equal(collided.name, 'Luma')
  assert.equal(collided.base.source, '/api/v1/file/luma-other.png')
  assert.equal(library.kits[existing.id], existing)
})
