...
test('Assets is a first-class tab with its own panel, not a fake active workspace', async () => {
  ...
  assert.match(panel, /tActivity\('inspector\.initiator'\)/)
  assert.match(panel, /tActivity\('inspector\.queue'\)/)
  assert.match(panel, /timing\.queue_ms/)
  assert.match(panel, /timing\.inference_ms/)
  assert.match(panel, /timing\.total_ms/)
  assert.doesNotMatch(panel, /queue_seconds/)
  assert.doesNotMatch(panel, /inference_seconds/)
  assert.doesNotMatch(panel, /total_seconds/)
