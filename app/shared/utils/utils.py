...
def get_video_frame(file_name: str, frame_no: int, return_last_if_missing: bool = False, target_fps = None,  return_PIL = True) -> torch.Tensor:
    ...
    # Detour HDR / anamorphic (SAR != 1) sources through ffmpeg with a proper
    # tonemap + SAR correction — cv2 would return PQ-encoded or squashed pixels.
    ...
    for i, img in enumerate(img_list if ignore_last_refs == 0 else img_list[:-ignore_last_refs]):
        ...
        if any_background_ref == 1 and i==0 or any_background_ref == 2:
            if outpainting_dims is not None and background_ref_outpainted:
                ...
                bg_scale = min(budget_height / height, budget_width / width)
                bg_new_h = int(height * bg_scale)
                bg_new_w = int(width * bg_scale)
                resized_image = img.resize((bg_new_w, bg_new_h), resample=Image.Resampling.LANCZOS)
                bg_top = (budget_height - bg_new_h) // 2
                bg_left = (budget_width - bg_new_w) // 2
                bg_canvas[bg_top:bg_top + bg_new_h, bg_left:bg_left + bg_new_w] = np.array(resized_image)
                resized_image = Image.fromarray(bg_canvas)
...
def fit_image_into_canvas(ref_img, image_size, canvas_tf_bg =127.5, device ="cpu", full_frame = False, outpainting_dims = None, outpainting_ratio = "", return_mask = False, return_image = False):
    ...
    if (ref_height, ref_width) == image_size and outpainting_dims  == None:
        ...
        if outpainting_dims != None:
            ...
            canvas_height, canvas_width, margin_top, margin_left =   get_outpainting_frame_location(final_height, final_width,  outpainting_dims, 1, outpainting_ratio = outpainting_ratio, source_height = ref_height, source_width = ref_width)
        else:
...
def prepare_video_guide_and_mask( video_guides, video_masks, pre_video_guide, image_size, current_video_length = 81, latent_size = 4, any_mask = False, any_guide_padding = False, guide_inpaint_color = 127.5, keep_video_guide_frames = [],  inject_frames = [], outpainting_dims = None, device ="cpu", frame_offset=1, pad_last_frame=False):
    ...
    for guide_no, (cur_video_guide, cur_video_mask) in enumerate(zip(video_guides, video_masks)):
        ...
        if pad_last_frame and src_video is not None and 0 < src_video.shape[1] < current_video_length:
            # Viggle's final control window must remain a valid native window.
            # Hold the actual last frame; the caller trims the generated tail.
            pad = src_video[:, -1:].expand(-1, current_video_length - src_video.shape[1], -1, -1)
        ...
        if any_guide_padding:
            if src_video is None:
                ...
                src_video = torch.cat([src_video, pad], dim=1)
        elif src_video is not None and src_video.shape[1] >= frame_offset:
            new_num_frames = (src_video.shape[1] - frame_offset) // latent_size * latent_size + frame_offset
