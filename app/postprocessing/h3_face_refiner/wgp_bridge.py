...
class H3FaceRefinerBridge(SimpleScaleSuffixMixin):
    ...
    def supports_loaded_model(self, spatial_upsampling, context, *, reference_images=None, **_kwargs) -> bool:
        if getattr(context.model, "fixed_prompt", None) is not None or self.validate_upsampling(spatial_upsampling, 0) or getattr(context.model, "refinement_api", None) != "masked_video_sigma_v1":
            return False
        if self.config()["model_mode"] == "current":
            return True
        return context.model_family == "h3_advanced"

    def query_download_defs(self) -> list[dict[str, Any]]:
        from models.h3_advanced.minimax_h3_handler import REF2VA_PRUNED_ARCHITECTURE, REPO_ID
        from models.h3_advanced.handler import family_handler
        from shared.wangp1272.assets import pin_downloads

        model_def = family_handler.query_model_def(REF2VA_PRUNED_ARCHITECTURE, {})
        return pin_downloads(family_handler.query_model_files(lambda filename: [os.path.basename(filename)], REF2VA_PRUNED_ARCHITECTURE, model_def) + [{
            "repoId": REPO_ID,
    ...
    def _private_model_def(wgp):
        ...
        # The local H3 Advanced adapter already selects the Q4 encoder.
        return model_def

    def _turbo_path(self, wgp, model_type=DEFAULT_MODEL_TYPE):
        from models.h3_advanced.minimax_h3_handler import REF_TURBO_LORA_KEY

        model_def = wgp.get_model_def(model_type)
        return wgp.resolve_lora_path(model_type, model_def[REF_TURBO_LORA_KEY])
    ...
    def download(self, process_files: Callable[..., Any], send_cmd=None, status_text: str | None = None, spatial_upsampling=None) -> bool:
        ...
        model_def = self._private_model_def(wgp)
        transformer = wgp.get_model_filename(DEFAULT_MODEL_TYPE, wgp.transformer_quantization, wgp.transformer_dtype_policy)
        wgp.download_models(transformer, DEFAULT_MODEL_TYPE, 0, -1)
        text_encoder_urls = wgp.get_model_recursive_prop(DEFAULT_MODEL_TYPE, "text_encoder_URLs", return_list=True)
        ...
        wgp.download_models(text_encoder, DEFAULT_MODEL_TYPE, 2, -1, force_path=model_def["text_encoder_folder"])
    ...
    def upscale(self, sample, spatial_upsampling, *, loaded_model_context=None, prompt="", seed=0, fps=24.0, frame_offset=0,
        ...
        hybrid_h3 = (loaded_model_context is not None and loaded_model_context.model_family == "h3_advanced"
        ...
        if loaded_model_context is None or hybrid_h3:
            ...
            if loaded_model_context.model_family == "h3_advanced":
        ...
        turbo_path = self._ensure_turbo(wgp) if loaded_model_context is None or loaded_model_context.model_family == "h3_advanced" else None
