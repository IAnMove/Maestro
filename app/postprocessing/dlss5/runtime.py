...
from shared.wangp1272.assets import dlss_guide_asset
...
def configure_depth_estimator(server_config):
    ...
    selected = server_config.get("depth_anything_v2_variant", "vitl")
    DEPTH_MODEL_VARIANT = selected if selected in {"vitb", "vitl"} else "vitl"
...
def _raft_model(device: torch.device):
    ...
    if key not in _RAFT_MODELS:
        ...
        weights = torch.load(dlss_guide_asset("flow/raft-things.pth"), map_location="cpu", weights_only=True)
...
def _depth_model(device: torch.device):
    ...
    if key not in _DEPTH_MODELS:
        ...
        if variant == "da3_metric_large":
            ...
            _DEPTH_MODELS[key] = DepthV2Annotator({"PRETRAINED_MODEL": dlss_guide_asset(filename), "MODEL_VARIANT": variant}, device=torch.device("cpu"))
        if device.type == "cuda":
