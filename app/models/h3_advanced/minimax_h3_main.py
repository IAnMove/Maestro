...
def model_factory(model_filename, text_encoder_filename, qkv_splitting, dtype=torch.bfloat16, VAE_dtype=torch.float32, save_quantized=False,
                  model_type="h3_advanced_fl2va", reference_mode=False, video_vae_filename=VIDEO_VAE_FILE,
    ...
    if save_quantized:
        raise ValueError("Checkpoint export is not supported by the H3 Advanced adapter")
    return pipeline
