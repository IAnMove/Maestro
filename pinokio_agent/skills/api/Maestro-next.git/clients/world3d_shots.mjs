...
import { numberedShots } from './shot_plan.mjs';
...
const shots=numberedShots(plan);
...
for(const {shot,number} of shots) {
 const stem=`clip-${String(number).padStart(2,'0')}`;
 ...
 // Confirm the full editor document, even when consecutive shots have no visible
 // number and share sources. Asset readiness alone cannot confirm a new import.
 const expected=await page.evaluate(async d=>{
  const {parseScene3DDocument}=await import('/src/features/scene3d/document.ts');
  const parsed=parseScene3DDocument(d);if(!parsed)throw Error('Invalid shot document');
 ...
 while(true) {
  ...
  const actual=JSON.stringify(JSON.parse(Buffer.concat(chunks).toString('utf8')));
  await download.delete();
  if(actual===expected)break;
  if(Date.now()>=deadline)throw Error(`Shot ${number} was not applied by the editor`);
 ...
 const record={clip:number,title:shot.title,start:shot.start,end:shot.end,saved:result,bytes:bytes.length,elapsedSeconds:(Date.now()-started)/1000};
